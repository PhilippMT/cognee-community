    ...
    try:
        ...
        if use_tavily:
            try:
                ...
                logger.info(
                    f"Retrieved {len(search_results)} web search results "
                    f"for thought {thought_id}"
                )

                # Persist web resources as nodes and connect to the thought
                for result in search_results:
                    ...
                    resource_id = str(uuid5(NAMESPACE_URL, url))
                    ...
                    resource_props = {
                        ...
                        "__class__": "WebResource",
                    }

                    await graph_engine.add_nodes([(resource_id, resource_props)])
                    ...
                    await graph_engine.add_edges([
                        (str(thought_id), resource_id, "enriched_by", edge_props)
                    ])
